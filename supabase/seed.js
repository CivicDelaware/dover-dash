...
async function main() {
  ...
  for (const record of airtableRecords) {
    ...
    // Structure: bp.profiles supports two formats:
    //   String format: { "homeowner": "Watch Out", "renter": "Helps You" }
    //   Object format: { "veteran": { "direction": "Helps You", "rationale": "..." } }
    // The object format carries a per-profile rationale. Both formats are handled below.
    // bp.wallet_rationale is the fallback rationale used when no per-profile rationale exists.
    if (bp && bp.profiles) {
      for (const [profileKey, dirValue] of Object.entries(bp.profiles)) {
        if (!dirValue) continue;

        // Normalize: extract direction string and rationale from either format
        const direction = (typeof dirValue === "object") ? dirValue.direction : dirValue;
        const rationale = (typeof dirValue === "object" && dirValue.rationale)
          ? dirValue.rationale
          : (bp.wallet_rationale || "");

        if (!direction) {
          console.warn(`    ⚠️  ${fullCode}/${profileKey}: no direction value, skipping`);
          continue;
        }
