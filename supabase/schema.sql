...
-- ═══════════════════════════════════════════════════════════════════════════
-- ADDITIONS — discovered after initial schema, added here for documentation.
-- These represent the actual live Supabase state as of July 2026.
-- All statements use IF NOT EXISTS / ON CONFLICT DO NOTHING so re-running
-- this file against an existing database is safe.
-- ═══════════════════════════════════════════════════════════════════════════


-- ── PROFILES: veteran and landlord ──────────────────────────────────────────
-- These two profiles were in bill_profiles.json and tracker.html from the start
-- but were missing from the original INSERT seed above.
-- Confirm they exist in Supabase → Table Editor → profiles before relying on them.
INSERT INTO profiles (key, label, emoji, description, sort_order) VALUES
  ('veteran',  'Veteran or Military',  '🎖️', '(active duty, reserve, veteran)',                             8),
  ('landlord', 'Landlord',             '🏢', '(residential rental property owner)',                         9)
ON CONFLICT (key) DO NOTHING;


-- ── BILLS: columns added after initial schema ────────────────────────────────
-- passed_date and nickname were added to Supabase but not tracked here.
-- get-bills.js queries both columns — if they are missing, the function
-- returns empty strings but does not error.
ALTER TABLE bills
...
-- ── CONFIG ──────────────────────────────────────────────────────────────────
-- Key-value table for runtime UI text and feature flags.
-- get-bills.js queries this table for ticker_message and other config keys.
-- The table was created directly in Supabase and was missing from this file.
...
-- Seed the keys get-bills.js expects. ON CONFLICT preserves any live values.
...
-- ── UPDATED VIEW: bills_with_profiles ───────────────────────────────────────
-- Rebuild the view to include the new passed_date and nickname columns.
CREATE OR REPLACE VIEW bills_with_profiles AS
SELECT
  b.id,
  b.session_number,
